...
if (defined('NV_IS_ADMIN_FULL_MODULE')) {
    ...
    if ($nv_Request->isset_request('submit', 'post') and $userid > 0) {
        ...
        if ($admin_module == 1 or $admin_module == 2) {
            ...
            $sql = 'SELECT id, title, parentid FROM ' . NV_PREFIXLANG . '_' . $module_data . '_cat ORDER BY sort ASC';
            $result_cat = $db->query($sql);
            while ($row = $result_cat->fetch()) {
                $admin_i = (in_array(in_array($row['id'], $array_admin) || $row['parentid'], $array_admin)) ? 1 : 0;
                if ($admin_i) {
                    $add_content_i = $edit_content_i = $del_content_i = 1;
                    $array_admin[] = $row['id'];
                } else {
                    $add_content_i = (in_array($row['id'], $array_add_content) || in_array($row['parentid'], $array_add_content)) ? 1 : 0;
                    $edit_content_i = (in_array($row['id'], $array_edit_content) || in_array($row['parentid'], $array_edit_content)) ? 1 : 0;
                    $del_content_i = (in_array($row['id'], $array_del_content) || in_array($row['parentid'], $array_del_content)) ? 1 : 0;
